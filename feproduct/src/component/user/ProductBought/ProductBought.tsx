...
const  ProductBought = ()  => {
    ...
    const increaseDetailCart = (id:number) => {
        let actions =  Actions.increaseProductCart(id);
        dispatch(actions);
    }
    return (
       <div className="ContainerCart">
           {
           cartOfUser.length === 0 ? "Có 0 sản phẩm trong giỏ hàng" : <div className="itemyoucart">
                 ...
                 return <div className="itemyoucart__detail" key={index}>
                            ...
                            <div className="itemyoucart__detail--name">
                                <div className="Product">
                                    ...
                                    <span className="Product__id">{res?.options[0]?.code}</span>
                                    <span className="Product__Insurance">Bảo hành: 24 Tháng</span>
                                </div>
                            </div>
                            <div className="itemyoucart__detail--price">
                                <del className="price__reduction">{res?.options[0]?.price}</del>
                                <span className="price__real">{res?.options[0]?.price}</span>
                                <span className="price__sum">Tổng: {res?.options[0]?.price}</span>
